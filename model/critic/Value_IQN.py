...
class Value_IQN(nn.Module):
    ...
    def __init__(self, args, environment, policy):
        ...
        self.net = nn.Sequential(DNN(self.state_dim, args.critic_hidden_dims, 1,
    ...
    def forward(self, feed_dict, tau_quantile):
        ...
        output = self.output_layer(
            torch.mul(V, tau_output)
        ).view(-1, 1)
        reg = get_regularization(self.net)
        return {'v': output, 'reg': reg}
    ...
    def get_sampled_Z(self, observation, confidences, policy_output):
        ...
        Z_tau_K = self(feed_dict={'state_emb': x}, tau_quantile=y)['q'].view(batch_size, K)
        return Z_tau_K